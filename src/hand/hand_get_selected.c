...
struct s_card_plane	*_hand_get_selected_card_plane(struct s_hand *hand)
{
	...
	{
		if (hand->card_selected[1] < 0 || hand->card_selected[1] >= (int)hand->shed_count/2)
			return (NULL);
...
struct s_card_desc	*hand_pop_selected_card(struct s_hand *hand)
{
	if (!hand)
		return (NULL);
	struct s_card_plane *selected_card_plane = _hand_get_selected_card_plane(hand);
	if (!selected_card_plane || !selected_card_plane->card_desc)
		return (NULL);
	struct s_card_desc *card_desc = selected_card_plane->card_desc;
	if (hand->status == HAND_DISPLAY_HAND)
	{
